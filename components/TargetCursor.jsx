...
const TargetCursor = ({
  ...
  useEffect(() => {
    ...
    // Initially hide target cursor until inside sectionSelector
    gsap.set(cursor, {
      opacity: 0,
      scale: 0.5,
      xPercent: -50,
    ...
    const moveHandler = (e) => {
      ...
      const inSection = Boolean(
        sectionSelector && targetElem && targetElem.closest && targetElem.closest(sectionSelector)
      );

      if (inSection && !isSectionVisibleRef.current) {
      ...
      } else if (!inSection && isSectionVisibleRef.current) {
    ...
    const mouseDownHandler = () => {
      if (!dotRef.current || !cursorRef.current || !isSectionVisibleRef.current) return;
