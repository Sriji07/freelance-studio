...
export default function ProjectCard({
    ...
    return (
        <div
            ...
            data-cursor="card"
            className={`tilted-card group relative flex h-full w-full flex-col justify-between cursor-pointer select-none rounded-[10px] bg-[#ffffff] p-5 sm:p-6 transition-all duration-300 ${
                isActive
        ...
        >
            ...
            <div className="mb-3.5 flex items-center justify-between">
                <div className="flex items-center gap-2">
                    ...
                    <span className="font-mono text-[11px] font-semibold tracking-wider text-[#111111]/80 uppercase">
                        {project.category}
                    </span>
                </div>
            ...
            {/* Main Visual Display Area - Minimalist architectural framing */}
            <div className="relative aspect-[4/3] w-full overflow-hidden rounded-[6px] bg-[#f5f1e8] border border-[#111111]/15 flex flex-col items-center justify-center p-5 transition-colors duration-300 group-hover:border-[#111111]/35">
                {/* Visual mock card inside */}
                <div className="relative flex flex-col items-center justify-center text-center">
                    <div className="mb-2.5 flex h-12 w-12 items-center justify-center rounded-[6px] bg-[#ffffff] border border-[#111111]/15 shadow-sm transition-transform duration-500 group-hover:scale-110">
                        <span className="font-mono text-sm font-bold text-[#111111]">
                            {project.title ? project.title.substring(0, 2).toUpperCase() : "WS"}
                        </span>
                    </div>
                    <span className="font-mono text-[9px] tracking-[0.2em] text-[#111111]/50 uppercase">
                        CASE STUDY
                    </span>
                    <h4 className="mt-1 text-base font-semibold tracking-tight text-[#111111] line-clamp-1">
                        {project.title}
                    </h4>
                </div>
            ...
            <div className="mt-4">
                <div className="flex items-baseline justify-between">
                    <div>
                        <h3 className="text-base sm:text-lg font-bold tracking-[-0.03em] text-[#111111]">
                            {project.title}
                        </h3>
                ...
                <div className="mt-3.5 flex items-center justify-between border-t border-[#111111]/10 pt-2.5">
                    <div className="flex items-center gap-1.5">
                        <span className={`h-1.5 w-1.5 rounded-full ${isActive || isHovered ? "bg-[#111111]" : "bg-[#111111]/25"}`} />
                        <span className="h-1.5 w-1.5 rounded-full bg-[#111111]/15" />
                        <span className="h-1.5 w-1.5 rounded-full bg-[#111111]/15" />
                    </div>
